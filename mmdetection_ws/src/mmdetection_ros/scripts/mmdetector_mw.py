...
MODEL_NAME =  'epoch_13.pth'    # yolo : 142, RetinaNet : 13
...
class Detector:
    ...
    def __init__(self, model):
        ...
        self.bbox_pub = rospy.Publisher("~bbox", BoundingBoxes, queue_size= 1)
        self.results_pub = rospy.Publisher("~results", Float64MultiArray, queue_size=1)
        # self.dummy_pub = rospy.Publisher('~counter', Int32)
    ...
    def projection(self,x,y,z) : 
        ...
        roll = -1.69392   # -1.69392
        pitch = -0.00428  # -0.00428
        yaw = 1.6        # 1.53333
        #distance_x =  -0.12   # -0.07
        distance_x = -0.07
        distance_y = -0.00484  #  -0.00484
        distance_z = 0.15394 #  0.15394
        ...
        intrinsic_matrix = np.array([[381.31,0.0,318.237],[0.0,381.018,242.887],[0, 0, 1]])
    ...
    #     result_y = (485.78 * (y / ((1 / np.cos(horizontal_theta)) *z))) + (0.4375 * 480)
    #     return result_x,result_y
    ...
    # NOTE: Hack the provided visualization function by mmdetection
        ...
        self.model.show_result(
                        ...
                        out_file = "/home/spalab/dummy1/"+str('{:04d}'.format(self.clock))+".png")
    ...
    def _pc_test_callback(self, msg):
        rospy.logdebug("Get a PC")
        if self._msg_lock.acquire(False):
            self._last_msg = msg
            pc = ros_numpy.numpify(msg)
            self.points_test = np.zeros((pc.shape[0],4))
            self.points_test[:,0] = pc['x']
            self.points_test[:,1] = pc['y']
            self.points_test[:,2] = pc['z']
            self.points_test[:,3] = 1
            self._msg_lock.release()
    ...
    def find_moving_object(self, xyz_arrs,object_list):
        ...
        for xyz_arr_idx in range(len(xyz_arrs)):
            ...
            for object in object_list:
                object_idx = np.where(object[:,-1]==xyz_arrs[xyz_arr_idx][-1])[0]
                if len(object_idx) != 0:
                    distance = abs(object[object_idx[0]][0] - xyz_arrs[xyz_arr_idx][0])
    ...
    def plot_points(self, dets, scan, image_np):
        ...
        # v1_range = (v1_tile > (dets[:, 1] + (height_arr/offset_v)).reshape(dets.shape[0], 1))\
        #     * (v1_tile < (dets[:, 3] - (height_arr/offset_v)).reshape(dets.shape[0], 1))
        ...
        for k in range(len(mask_arr)):
            ...
            try:
                ...
                pass
        # try:
        #     np.save("/home/spalab/mmdetection_ws/ros_in_box_points.npy", in_box_points)
        #     np.save("/home/spalab/mmdetection_ws/ros_clustering_points.npy", before_clustered_points)
        # except:
        #     pass
    ...
    def visualize(self, projected_scan, dets, image):
        ...
        #cv2.imwrite("/home/spalab/mmdetection_ws/demo_image.jpg",image)
        ...
        cv2.imwrite(f"/home/spalab/mmdetection_ws/track_projected_pcd_in_detections/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        #cv2.imwrite(f"/home/spalab/mmdetection_ws/image_with_bbox/{str(self.frame_count).zfill(6)}.jpg",detected_img)
        ...
        cv2.imwrite(f"/home/spalab/mmdetection_ws/track_projected_pcd/{str(self.frame_count).zfill(6)}.jpg",image)
    ...
    def visualize_moving_object(self, xyz_arrs_with_move, image_np):
        ...
        for idx in range(len(projected_scan[:,0])):
            ...
            try:
                ...
                cv2.putText(image_np,str(0),(int(projected_scan[idx,0]),int(projected_scan[idx,1])),cv2.FONT_HERSHEY_PLAIN,1,(0, 0, 255),2)
                cv2.putText(image_for_depth,str(np.round(distance,1)),(int(projected_scan[idx,0]),int(projected_scan[idx,1])),cv2.FONT_HERSHEY_PLAIN,1,(255, 0, 255),2)
                
                #cv2.putText(image_for_depth,str(np.round(xyz_arrs_with_move[idx,0],2)),(int(projected_scan[idx,0]),int(projected_scan[idx,1])),cv2.FONT_HERSHEY_SCRIPT_SIMPLEX,1,(255, 0, 255),5)
                
        cv2.imwrite(f"/home/spalab/mmdetection_ws/track_points/{str(self.frame_count).zfill(6)}.jpg", image_np)
        cv2.imwrite(f"/home/spalab/mmdetection_ws/track_depth/{str(self.frame_count).zfill(6)}.jpg", image_for_depth)
    ...
    def visualize_trackids(self,track_bbs_ids,image_np):
        ...
        cv2.imwrite(f"/home/spalab/mmdetection_ws/track_ids/{str(self.frame_count).zfill(6)}.jpg", image_np)
    ...
    def run(self):
        ...
        if not self._is_service:
            ...
            # image_sub = rospy.Subscriber("~/darknet_ros/detection_image", Image, self._image_callback, queue_size=1)
            ...
            pointcloud_sub = rospy.Subscriber("/velodyne_points_test",PointCloud2, self._pc_test_callback, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if msg is not None:
                ...
                # image_np_behind = image_np
                
                # imaeg_np_behind = np.asarray(im_behind)
                # image_np_behind = cv2.cvtColor(image_np_behind,cv2.COLOR_BGR2RGB)
                ...
                # poin = np.load("/home/spalab/mmdetection_ws/ros_points.npy")
                ...
                # cv2.imwrite(f"/home/spalab/mmdetection_ws/{str(self.frame_count).zfill(6)}.jpg",image_np)
                detections = dets[:,:6]
                ...
                #np.save("/home/spalab/mmdetection_ws/ros_points.npy",masked_points)
                ...
                # np.save("/home/spalab/mmdetection_ws/ros_in_image_points.npy",masked_points[point_in_image_list])
                ...
                #np.save("/home/spalab/mmdetection_ws/ros_points.npy",points_masked)
                ...
                #xyz_arrs_with_move : xyz_arr : x,y,z,cls_idx,track_id,xmin,ymin,xmax,ymax,move_or_not (9)
                ...
                for num_ in range(len(final_results)):
                    ...
                    # final_results[num_][-2] = CLASSES[int(final_results[num_][-2])]
                    # final_results[num_][-1] = int(final_results[num_][-1])
...
def main(args):
    ...
    #     print("ShutDown")
    # p1 = multiprocessing.Process(target=obj.run(), args=())
    # print("start")
