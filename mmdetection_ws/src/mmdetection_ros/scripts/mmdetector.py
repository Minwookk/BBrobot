...
MODEL_NAME =  'epoch_12.pth'    # yolo : 142, RetinaNet : 13
...
class Detector:
    ...
    def projection(self,x,y,z) : 
        ...
        roll = -1.6090   # -1.69392
        pitch = -0.0028  # -0.00428
        yaw = -1.7        # 1.522
        distance_x =  -0.1   # -0.2011
        distance_y = 1  #  -0.0092
        distance_z = 2.5 #  0.3395
...
#            
            ...
            try:
                ...
                projected_cluster_scan = np.zeros_like(scan[list_[min_depth][0],:2])
                projected_cluster_scan[:,0], projected_cluster_scan[:,1] = self.projection(scan[list_[min_depth][0],0], scan[list_[min_depth][0],1], scan[list_[min_depth][0],2])
                   
                projected_cluster_scan_list.append(projected_cluster_scan)
                # projected_cluster_scan = np.zeros_like(scan[list_[max_index][0],:2])
                # projected_cluster_scan[:,0], projected_cluster_scan[:,1] = self.projection(scan[list_[max_index][0],0], scan[list_[max_index][0],1], scan[list_[max_index][0],2])
                # projected_cluster_scan[:,1] = projected_cluster_scan[:,1] + 35
                # projected_cluster_scan_list.append(projected_cluster_scan)
                ##
...
#                    xyz_arrs = np.array((x, y, z, cls_idx, track_id)).reshape(1, -1)
        ...
        #self.visualize_clustered_pointsv2(projected_cluster_scan_list, dets, image_np)
...
##0406
    ...
    def visualize_clustered_point(self, projected_scans, dets, image):


        copied_image = copy.deepcopy(image)
        for projected_scan in projected_scans:
        ...
        #cv2.imwrite(f"/media/patrol2/SAMSUNG/demo/track_clustered_points_in_detections/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        cv2.imwrite(f"/home/patrol2/mmdetection_ws/track_clustered_points_in_detections/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        #cv2.imwrite(f"/home/patrol2/mmdetection_ws/{str(self.frame_count).zfill(6)}.jpg",detected_img)

    def visualize_clustered_pointsv2(self, projected_scans, dets, image):
        ...
        cv2.imwrite(f"/home/patrol2/mmdetection_ws/track_projected_clustered_pcd/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        cv2.imwrite(f"/home/patrol2/mmdetection_ws/track_projected_clustered_pcd_in_detections/{str(self.frame_count).zfill(6)}.jpg",image)        
    ...
    def visualize(self, projected_scan, dets, image):
        ...
        for det in dets:
            ...
            for idx in range(len(projected_scan[:,0])):
                if projected_scan[:,0][idx]>det[0] and projected_scan[:,0][idx]<det[2] and projected_scan[:,1][idx]>det[1] and projected_scan[:,1][idx]<det[3]:
                    cv2.circle(copied_image,(int(projected_scan[:,0][idx]),int(projected_scan[:,1][idx])),2,(255,255,0),-1)
        #cv2.imwrite(f"/media/patrol2/SAMSUNG/demo/track_projected_pcd/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        cv2.imwrite(f"/home/patrol2/mmdetection_ws/bbox/{str(self.frame_count).zfill(6)}.jpg",copied_image)
        ...
        #cv2.imwrite(f"/media/patrol2/SAMSUNG/demo/track_projected_pcd_in_detections/{str(self.frame_count).zfill(6)}.jpg",image)
        cv2.imwrite(f"/home/patrol2/mmdetection_ws/bbox2/{str(self.frame_count).zfill(6)}.jpg",image)
    ...
    def visualize_moving_object(self, xyz_arrs_with_move, image_np):
        ...
        cv2.imwrite(f"/media/patrol2/SAMSUNG/demo/track_ids/{str(self.frame_count).zfill(6)}.jpg", image_np)
        cv2.imwrite(f"/media/patrol2/SAMSUNG/demo/track_depth/{str(self.frame_count).zfill(6)}.jpg", image_for_depth)
    ...
    def run(self):
        ...
        if not self._is_service:
            ...
            # detections_behind_sub = rospy.Subscriber("/darknet_ros/bounding_boxes", BoundingBoxes, self._yolo_behind_callback, queue_size=1) 
        else:
        ...
        while not rospy.is_shutdown():
            ...
            if msg is not None:
                ...
                # vis1
                #self.visualize_trackids(track_bbs_ids,image_np_trackids)  #전방
